...
export async function addMemoryToLorebook(memoryResult, lorebookValidation, options = {}) {
    ...
    try {
        ...
        populateLorebookEntry(newEntry, effectiveMemoryResult, entryTitle, lorebookSettings, lorebookValidation.data);
...
/**
 * Drops keywords from a newly-generated entry that an existing entry in the
 * same lorebook already claims verbatim (case-insensitive exact match).
 * PHA-1846/PR#24's dedupeOverlappingKeys only looks within one entry's own
 * key list (substring overlap), so two different entries independently
 * generated by separate one-shot LLM calls (stmemory.js has no cross-entry
 * visibility) can still both claim e.g. "Brotherhood of Steel" and co-fire.
 * Always keeps at least one keyword so a new entry never ships unretrievable.
 * @param {string[]} keys
 * @param {Object} lorebookData - existing lorebook (world info) data
 * @returns {string[]}
 */
function dedupeAgainstExistingEntries(keys, lorebookData) {
    const list = Array.from(new Set((keys || []).map(k => String(k || '').trim()).filter(Boolean)));
    if (list.length === 0 || !lorebookData?.entries) return list;

    const claimed = new Set();
    for (const existing of Object.values(lorebookData.entries)) {
        for (const k of (existing?.key || [])) {
            claimed.add(String(k || '').trim().toLowerCase());
        }
    }
    if (claimed.size === 0) return list;

    const filtered = list.filter(key => !claimed.has(key.toLowerCase()));
    return filtered.length > 0 ? filtered : list;
}

function populateLorebookEntry(entry, memoryResult, entryTitle, lorebookSettings, lorebookData) {
    // Core content and keywords
    entry.content = memoryResult.content;
    entry.key = dedupeAgainstExistingEntries(dedupeOverlappingKeys(memoryResult.suggestedKeys || []), lorebookData);
